...
class ALAudioDevice(object):
    ...
    def setClientPreferences2(self, name, sampleRate, channelssession, deinterleaved):
        ...
        :param int channelssession: An int (defined in ALSoundExtractor) indicating which microphones data will be send to the process function. ALLCHANNELS, LEFTCHANNEL, RIGHTCHANNEL, FRONTCHANNEL, REARCHANNEL are the session currently supported.
        ...
        if not self.proxy:
            self.proxy = self.session.service("ALAudioDevice")
        return self.proxy.setClientPreferences(name, sampleRate, channelssession, deinterleaved)

#!/usr/bin/env python
# Class autogenerated from /home/sam/Downloads/aldebaran_sw/nao/naoqi-sdk-2.1.4.13-linux64/include/alproxies/alsittingpeopledetectionproxy.h
...
class ALSittingPeopleDetection(object):
    def __init__(self, session):
        self.proxy = None 
        self.session = session

    def force_connect(self):
        self.proxy = self.session.service("ALSittingPeopleDetection")
    ...
    def getSittingThreshold(self):
        """Gets sitting threshold

        :returns float: Maximum face height in meter
        """
        if not self.proxy:
            self.proxy = self.session.service("ALSittingPeopleDetection")
        return self.proxy.getSittingThreshold()
    ...
    def setSittingThreshold(self, threshold):
        """Sets sitting threshold

        :param float threshold: float, face height (in meter) from the ground under which we consider thatsomeone is sitting.
        """
        if not self.proxy:
            self.proxy = self.session.service("ALSittingPeopleDetection")
        return self.proxy.setSittingThreshold(threshold)

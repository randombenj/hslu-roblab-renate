#!/usr/bin/env python
# Class autogenerated from /home/sam/Downloads/aldebaran_sw/nao/naoqi-sdk-2.1.4.13-linux64/include/alproxies/alvisualspacehistoryproxy.h
...
class ALVisualSpaceHistory(object):
    def __init__(self, session):
        self.proxy = None 
        self.session = session

    def force_connect(self):
        self.proxy = self.session.service("ALVisualSpaceHistory")
    ...
    def getGridPrecision(self):
        """Gets grid precision

        :returns float: Grid precision
        """
        if not self.proxy:
            self.proxy = self.session.service("ALVisualSpaceHistory")
        return self.proxy.getGridPrecision()
    ...
    def resetGrid(self):
        ...
        if not self.proxy:
            self.proxy = self.session.service("ALVisualSpaceHistory")
        return self.proxy.resetGrid()

    def setGridPrecision(self, precision):
        """Sets grid precision

        :param float precision: float, ratio between camera field of view and grid cell field of view in each dimension. Setting the grid precision will reset the grid.
        """
        if not self.proxy:
            self.proxy = self.session.service("ALVisualSpaceHistory")
        return self.proxy.setGridPrecision(precision)
